...
def recognize_command(text):
    text = text.lower()
    print(f"Debug: Recognizing command from text: '{text}'")  # Debug statement
    ...
    for action, variations in commands.items():
        if any(variation in text for variation in variations):
            print(f"Debug: Command recognized as '{action}'")  # Debug statement
            return action

    print("Debug: No command recognized; defaulting to general conversation")  # Debug statement
...
async def process_input(text, history):
    ...
    print(f"Debug: Processed command: {command}")  # Debug statement
    ...
    if command == "CHECK_SCHEDULE":
        ...
        print("Debug: TURN_OFF command triggered. Initiating shutdown sequence.")  # Debug statement
        ...
        # Stop the event loop explicitly and exit
        asyncio.get_running_loop().stop()
        sys.exit(0)
    else:
        print("Debug: Entering GENERAL_CONVERSATION mode.")  # Debug statement
        response = await chat(f"{base_prompt} {text}", history)
